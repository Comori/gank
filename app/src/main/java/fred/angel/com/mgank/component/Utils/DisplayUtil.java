...
public class DisplayUtil {
    ...
    private static final DisplayMetrics metrics = IApplication.INSTACE.getResources().getDisplayMetrics();
...
//            statusHeight = rect.top;
    ...
    public static int getKeyboardHeight(){
        if(keyboardHeight <= 0){
            Integer h = LocalDataFactory.<Integer>getManager(LocalDataFactory.LocalDataType.SP)
                    .find(Constant.LocalCacheKey.SP_KEYBOARD_HEIGHT);
            keyboardHeight = h==null?0:h;
        }
        return keyboardHeight;
    }

    public static void setKeyboardHeight(int keyboardHeight) {
        DisplayUtil.keyboardHeight = keyboardHeight;
        LocalDataFactory.getManager(LocalDataFactory.LocalDataType.SP)
                .put(Constant.LocalCacheKey.SP_KEYBOARD_HEIGHT,keyboardHeight);
    }
}